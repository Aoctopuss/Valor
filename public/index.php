...
function resolveCategory(PDO $pdo, int $userId): ?int {
...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST['new_entry'])) {
    ...
    $category_id = resolveCategory($pdo, $user_id);
    ...
    <div class="w-1/2 mx-8">
        <form class="bg-text-body border border-neutral-800 rounded-md">
            <label class="flex flex-row items-center m-3 gap-2">
                ...
                <input id="search" class="w-full bg-transparent text-white outline-none focus:ring-0 placeholder:text-neutral-600 text-sm" placeholder="Search vault..." />
            </label>
        </form>
    </div>
